...
- 图片/PDF/视频理解：图片会交给千问视觉模型提取文字和病例信息；PDF 会先在浏览器转成页面截图；视频会在浏览器抽取关键帧后交给千问分析。
...
当前实现不会把 20MB 视频或大 PDF 整段传给后端，而是在浏览器本地抽取视频关键帧或 PDF 页面截图，再把压缩后的图片帧发送给后端调用千问视觉模型。这样速度更快，也避免 Cloudflare Functions 处理大文件上传。

## 当前知识库能力

- 文本、标题、标签可以直接检索。
- `.txt`、`.md`、`.json` 文件会自动读取为文本。
- 图片、PDF 和视频会保存文件元信息；为了避免大文件上传很慢，PDF 和视频默认不整段保存到后端。
- 配置千问视觉模型后，图片、PDF 页面和视频帧会生成“图片/视频 AI 解析”文字，并写入知识库参与检索。
- 知识库命中后，会优先让 AI 基于命中的麦芽知识库资料做总结回答。
- 匹配分只代表问题和资料之间的关键词重合程度，不代表医学可信度、答案质量或百分制分数。
- 默认只有匹配分达到 10 及以上才算知识库命中；低于 10 会自动调用外部 AI API。

如果要让大量图片、PDF 和视频也能更稳定地参与问答，后续需要加 OCR 队列、语音转文字和向量数据库。
...
没有配置千问视觉环境变量时，图片/PDF/视频解析接口会返回明确错误，普通文字知识库和聊天仍可使用。
